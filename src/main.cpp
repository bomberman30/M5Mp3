...
void loop()
{
    // if press reset timer for dim display
    ...
    // chack time to dim display
    if (!screenOff && (millis() - lastInputTime > 10000)) // 10000 is 10 second
    {
    ...
    while (file && fileCount < MAX_FILES)
    {
        ...
        else
        {
            ...
            if (hasExtension(name, ".mp3"))
            {
                ...
                M5Cardputer.Display.print("Scaned File" + CutrrentScan);
                String fullPath = String(dirname);
                if (!fullPath.endsWith("/"))
                    fullPath += "/";
                fullPath += name;
